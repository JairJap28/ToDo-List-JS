//Check if there is something in the localStorager
//if so, add it to the object, otherwise create the object
...
//render the data from the localStorage
rederToDoList();
...
function removeItem(e){
    ...
    //Delete the item checking the list
    if(id === 'todo'){
...
function completItem(){
    ...
    //This var will be used to store the text
    var value = item.innerText;
    var target;

    //Check if the item should be added to completed list or re-added to the to-do list
    //at the same time pass the data to the opposite list
    if(id === 'todo'){
        data.todo.splice(data.todo.indexOf(value), 1);
